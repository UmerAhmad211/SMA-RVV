...
void
q15_axpy_ref(const int16_t *a, const int16_t *b, int16_t *y, int n,
	     int16_t alpha)
{
	for (int i = 0; i < n; ++i) {
...
void
q15_axpy_rvv(const int16_t *a, const int16_t *b, int16_t *y, int n,
	     int16_t alpha)
{
	...
	for (size_t vl; n > 0; n -= vl, a += vl, b += vl, y += vl) {
		...
		vint16m1_t ald = __riscv_vle16_v_i16m1(a, vl);
		vint16m1_t bld = __riscv_vle16_v_i16m1(b, vl);

		vint32m2_t acc = __riscv_vwmacc_vx_i32m2(
			__riscv_vwcvt_x_x_v_i32m2(ald, vl), alpha, bld, vl);
		vint16m1_t res = __riscv_vnclip_wx_i16m1(acc, 0, 2, vl);

		__riscv_vse16_v_i16m1(y, res, vl);
	}
#endif
}
...
static int
verify_equal(const int16_t *ref, const int16_t *test, int n, int32_t *max_diff)
{
	int ok = 1;
	int32_t md = 0;
	for (int i = 0; i < n; ++i) {
